  ...
  return (
    <section>
      ...
      <div style={{ marginBottom: 12, display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        <span>Totale a credito: <strong style={{ color: colors.danger }}>{fmtEuro(total)}</strong></span>
            ...
            <tbody>
              {customers.map((c) => (
                <tr key={c.id} onClick={() => setSelected(c.id)}
                  ...
                  <td style={{ padding: 8, textAlign: 'right', color: c.balanceCents > 0 ? colors.danger : colors.success }}>{fmtEuro(c.balanceCents)}</td>
                </tr>
              ))}
